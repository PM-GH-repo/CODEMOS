...
class Emperor:
	...
	def chooseSimulationToLaunch(self):
		...
		if sim:		print("     Next: "+sim.name)
		else:		print("No simulation to be launched")
	...
	def runAllSimulations(self,nPROC=0,strategy="from_first",functions=[]):
		...
		while(chosen_simulation):
			...
			print("Launching: "+chosen_simulation.name)
			...
			while self.simulations_to_be_launched and not chosen_simulation:		#no simulation is selected although there are simulations to be calculated:
				#HERE I SHOULD CHECK THAT SOME SIMULATION IS RUNNING AND EXIT IF NOT!!!	#the only plausible reason for this is that simulations, on which all the waiting simulatinons depend, are still running.
		...
		if not aux_string:
			...
			for ii in self.files_copy:
				if not os.path.exists(ii[0]):
					#print("Warning, file \"%s\" indexed for copy does not yet exist. This can make sence if it is outcome of a prerequisite simulation"%ii)
			...
			aux_string=aux_string.encode(encoding = 'UTF-8', errors = 'strict')
